...
ItgJs.fn_numberOnly = function (evt) {
    ...
    if ((code > 34 && code < 41) || (code > 47 && code < 58) || (code > 95 && code < 106) || code == 8 || code == 9 || code == 13 || code == 46 || (evt.ctrlKey && (code == 67 || code == 86)) /* ctrl + c, ctrl + v*/) {
        return true;
    }
    if (evt.returnValue) {
        alert("숫자만 입력가능 합니다.");
...
ItgJs.fn_phone = function (evt) {
    evt = evt || window.event;
    var code = evt.keyCode || evt.which;
    if ((code > 34 && code < 41) || (code > 47 && code < 58) || (code > 95 && code < 106) || code == 8 || code == 9 || code == 13 || code == 46 || code == 189 || code == 109 || (evt.ctrlKey && (code == 67 || code == 86)) /* ctrl + c, ctrl + v*/) {
        return true;
    }
    if (evt.returnValue) {
...
ItgJs.fn_checkPass = function (pass1, pass2) {
    ...
    if (pass1.length < 9) {
        return "비밀번호는 영문, 숫자, 특수문자의 조합으로 9자리 이상 입력해주세요.";
    }
    ...
    if (SamePass_0 > 0) {
        return "동일문자를 3회 이상 연속으로 사용할 수 없습니다.";
    }
...
ItgJs.fn_datePicker = function (dateId, format, startDate) {
    if (ItgJs.fn_isEmpty(format)) {
        format = "yyyy-mm-dd";
    }
    if (ItgJs.fn_isEmpty(startDate)) {
        startDate = "";
    }
...
ItgJs.fn_datePickerRange = function (startId, endId) {
    ...
    $(startId)
        ...
        .on("changeDate", function (e) {
            $(endId).datepicker("setStartDate", $(startId).val());
        });
    ...
    $(endId)
        ...
        .on("changeDate", function (e) {
            $(startId).datepicker("setEndDate", $(endId).val());
        });
...
ItgJs.selectBoxLocalNumber = function (id, value) {
    var localNumberArr = ["02", "051", "053", "032", "062", "042", "052", "031", "033", "043", "041", "063", "061", "054", "055", "064", "070"];
...
ItgJs.selectBoxMobileLocalNumber = function (id, value) {
    var localNumberArr = ["010", "011", "016", "017", "018", "019"];
    $("#" + id).append("<option value=''>선택</option>");
    ItgJs.selectBoxForeach(localNumberArr, id, value);
};

ItgJs.selectBoxEmail = function (id, value) {
    var emailArr = ["daum.net", "hanmail.net", "paran.com", "hotmail.com", "chollian.net", "dreamwiz.com", "empal.com", "hanmir.net", "hitel.com", "korea.com", "lycos.co.kr", "nate.com", "naver.com", "netian.net", "gmail.com"];
...
ItgJs.showLoading = function () {
    ...
    spinner = '<div class="lds-css ng-scope">' + '<div class="loading-spiner" style="100%;height:100%"><img src="/cubersc/common/img/loading-bar.gif"/></div></div>';
    /*		if(isOldIe){
		}else{
