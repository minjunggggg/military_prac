...
(function (factory) {
    ...
    $.fn.dateRangePicker = function (opt) {
        ...
        /* //select-option 태그 만들기 1/3 (스타일 변경시 이곳 참조 20231103_shw) 참조 옵션 monthSelect:true, yearSelect:true current +- '숫자'(yearSelectRange) 만큼 출력됨.*/
        /* yearSelectRange defalut : 5 년도선택에서 선택박스에 보여지는 전후년도 2023-11-17*/
        if (opt.yearSelect && typeof opt.yearSelect === "boolean") {
            opt.yearSelect = function (current) {
                return [current - opt.yearSelectRange, current + opt.yearSelectRange];
            };
        }
        /* //select-option 태그 만들기 1/3 (스타일 변경시 이곳 참조 20231103_shw) 참조 옵션 monthSelect:true, yearSelect:true */
        ...
        /* select-option 태그 만들기 2/3 (스타일 변경시 이곳 참조 20231103_shw) 참조 옵션 monthSelect:true, yearSelect:true */
        function showMonth(date, month) {
            date = moment(date).toDate();
            var yearElement = generateYearElement(date, month);
            var monthElement = generateMonthElement(date, month);

            box.find("." + month + " .month-name")
                .html(yearElement + "&nbsp " + monthElement)
        ...
        function generateYearElement(date, month) {
            ...
            var nonSelectableMonth = '<div class="month-element">' + currentYear + "년&nbsp;</div>";
        ...
        /* select-option 태그 만들기 3/3 (스타일 변경시 이곳 참조 20231103_shw) 참조 옵션 monthSelect:true, yearSelect:true */
        ...
        function generateSelect(name, data) {
            ...
            for (var i = 0, l = data.length; i < l; i++) {
                var item = data[i];
                select += '<option value="' + item.value + '"' + (item.selected ? " selected" : "") + (item.disabled ? " disabled" : "") + ">" + item.text + addstr + "</option>";
            ...
            select += "</select>" + current + addstr + "</div>";

            return select;
        }

        /* /select-option 태그 만들기 3/3 (스타일 변경시 이곳 참조 20231103_shw) */
